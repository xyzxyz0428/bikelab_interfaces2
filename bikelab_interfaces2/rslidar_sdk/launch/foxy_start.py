...
def install_cyclone_dds():
    ...
    result = subprocess.run(['dpkg', '-s', 'ros-foxy-rmw-cyclonedds-cpp'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if result.returncode == 0:
        ...
        try:
            ...
            subprocess.run(['sudo', '-S', 'apt-get', 'install', '-y', 'ros-foxy-rmw-cyclonedds-cpp'], input=password.encode(), check=True)
...
def generate_launch_description():
    if os.getenv('ROS_DISTRO') == 'foxy':
        print("Detected ROS 2 foxy. Checking DDS...")
        install_cyclone_dds()
